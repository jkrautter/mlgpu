...
def classify(images):
    ...
    with tf.name_scope("conv1"):
        weights = tf.Variable(tf.truncated_normal([5, 5, 1, 6], stddev=0.1, dtype=tf.float32), name='weights')
        biases = tf.Variable(tf.constant(0.1, shape=[6], dtype=tf.float32), name='biases')
        conv1 = tf.nn.conv2d(images, weights, strides=[1, 1, 1, 1], padding='VALID')
    ...
    with tf.name_scope("conv2"):
        weights = tf.Variable(tf.truncated_normal([6, 6, 6, 16], stddev=0.1, dtype=tf.float32), name='weights')
        biases = tf.Variable(tf.constant(0.1, shape=[16], dtype=tf.float32), name='biases')
        conv2 = tf.nn.conv2d(hidden2, weights, strides=[1, 1, 1, 1], padding='VALID')
    ...
    with tf.name_scope("conv3"):
        weights = tf.Variable(tf.truncated_normal([4, 4, 16, 512], stddev=0.1, dtype=tf.float32), name='weights')
    ...
    with tf.name_scope('hidden6'):
        reshape = tf.reshape(hidden5, [BATCH_SIZE, 512])
        weights = tf.Variable(tf.truncated_normal([512, 512], stddev = 1.0 / math.sqrt(float(512))), name='weights')
        biases = tf.Variable(tf.constant(0.1, shape=[512], dtype=tf.float32), name='biases')
    ...
    with tf.name_scope('softmax_linear'):
        weights = tf.Variable(tf.truncated_normal([128, NUM_CLASSES], stddev = 1.0 / math.sqrt(float(84))), name='weights')
        biases = tf.Variable(tf.constant(0.1, shape=[NUM_CLASSES], dtype=tf.float32), name='biases')
        logits = tf.matmul(hidden7, weights) + biases
